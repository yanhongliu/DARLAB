...
struct tree_opt_pass pass_plugin =
{
  ...
  PROP_gimple_any | PROP_ssa | PROP_cfg | PROP_referenced_vars,
...
void pre_compile_plugins(void)
{
  RUN_ALL_FUNCS(pre_compile);
}
...
void post_compile_plugins(void)
{
  RUN_ALL_FUNCS(post_compile);
}
...
/*
static void diag_path_and_args(const struct plugin_handle* const handle)
{
	int i;
	...
	for(i = 0; i < handle->argc; i++) {
		if(handle->argv[i].value)
			printf("%d KEY [%s] VALUE [%s]\n", i, handle->argv[i].key, handle->argv[i].value);
		else
			printf("%d KEY [%s] VALUE [null]\n", i, handle->argv[i].key);
	}
}
*/
...
void register_tree_plugin(const char* spec)
{
  ...
  lt_ptr pre_compile_symbol;
  lt_ptr pre_tu_symbol;
  lt_ptr run_symbol;
  lt_ptr post_tu_symbol;
  lt_ptr post_compile_symbol;
  ...
  pre_compile_symbol	= lt_dlsym(handle, "pre_compile");
  pre_tu_symbol		= lt_dlsym(handle, "pre_translation_unit");
  run_symbol		= lt_dlsym(handle, "run");
  post_tu_symbol	= lt_dlsym(handle, "post_translation_unit");
  post_compile_symbol	= lt_dlsym(handle, "post_compile");
  
  if(!(pre_compile_symbol || post_compile_symbol ||
       pre_tu_symbol || post_tu_symbol ||
       run_symbol))
    {
      warning(0, "Plugin %s does not have any exported functions.", plugins[num_plugins].path);
      return;
    }

  plugins[num_plugins].pre_compile		= (void (*)(void))pre_compile_symbol;
  plugins[num_plugins].pre_translation_unit	= (void (*)(void))pre_tu_symbol;
  plugins[num_plugins].run			= (void (*)(void))run_symbol;
  plugins[num_plugins].post_translation_unit	= (void (*)(void))post_tu_symbol;
  plugins[num_plugins].post_compile		= (void (*)(void))post_compile_symbol;
