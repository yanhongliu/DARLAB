...
class Repr:
    ...
    def get_basic_analyzed(self,repr,driver):
#        if not hasattr(repr,"_ares"):
#            session=analysis.source.AnalysisSession()
#            session.librarypath='lm_py\\analysis\\'
#            session.codepath=os.path.dirname(driver)
#            session.simplify_files=True
#            sys.setrecursionlimit(400000)
#            vc=analysis.alias.visitCreator()
#            visit=vc.visitFunc()
#            graph=vc.graph
#            ad=session.processFile_to_CFG(driver,cfg_visitor=visit,only_source_ast=True,post_traversal=False) #computes graph
...
#            analysis.alias.GoyalTrace(graph,w,storage,maxallowed=1).doAnalysis()
#            res=storage.dump()
#        else:
#            res=repr._ares
        ast= ast_from_code(ast2string(repr))
#        session=analysis.source.AnalysisSession()
#        session.path=self.filename
#        session.librarypath='lm_py/analysis'
#        ast=session.processAST(ast)
#        ast._ares=res
        #pdb.set_trace()
        return ast
