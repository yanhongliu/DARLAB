...
def spawn_local_handler():
    ...
    proc=Popen([python,GRAPHSERVER,"--stdio"],shell=True,bufsize=0,stdin=PIPE,stdout=PIPE)
    child_in, child_out = proc.stdin, proc.stdout
    io = msgstruct.FileIO(child_out, child_in)
    io.printout=True
    #os.spawnl(os.P_NOWAIT,python,'-u','"%s"'%GRAPHSERVER,'1455')
    #return spawn_graphserver_handler(("127.0.0.1",1455))
    return io
