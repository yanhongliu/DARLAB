...
class AnalysisVisitor(ASTVisitor):
    ...
    def __init__(self, session=None, filename=None, module_name=None):
        ...
        self.queueSourcePoint=[]
        self.queueReturn=[]
        self.functionexit=set()
        self.mustgotonext=[]
    ...
    def default(self, node, handler):
        ...
        elif analysis.operators.is_list_operator(node):
            self.process_list_operator(node, handler)
    ...
    def visitAssName(self, assname, handler):
        ...
        if assname.name=="eval_code_in_interpreter":          
            exec assname._parent.expr.value
        #if "163" in assname.name:
        #  pdb.set_trace()
    ...
    def visitCallFunc(self, callfunc, handler):
            ...
            if attributes != []:
                for attribute in attributes:
                    ...
                    if isinstance(attribute, compiler.ast.Function):
                        ...
                        try:
                            spec=self.call_specialisation(callfunc, attribute, args,
                                "context", handler,
                                callfunc.star_args, callfunc.dstar_args)
                            #pdb.set_trace()
                            #if callfunc.node is not None and spec is not None:
                            #    callfunc.node.name=spec.name
                            attributes_found = 1
        ...
        if not attributes_found:
            scallfunc=ast2string(callfunc)
            cf=callfunc
            if hasattr(cf,"_original"):
                cf=cf._original
            if hasattr(cf,"lineno"):
                scallfunc += " at line: %d"%cf.lineno
            callstack = "->".join([ast2string(i) for i in self.current_callers[:]])
            hist="\n".join([str(i) for i in undesirable_targets])
            #pdb.set_trace()
            raise NoTargetsError, "\nCannot call: %s -> %s"%(scallfunc, hist)

#    def visitBitand(self,nodes,handler):
#        pdb.set_trace()
    ...
    def visitKeyword(self, keyword, handler):
        #Propagate type/alias info to keyword from what it contains
        self.dispatch(keyword.expr,handler)
        self.graph.merge(keyword,keyword.expr)
    ...
    def visitConst(self, const, handler):
        ...
        elif type(const.value) == type(None):
            definitions = handler.builtins["none"]
        else:
    ...
    def visitListComp(self,comp,handler):
        #pdb.set_trace()
        first=False
        for c in comp.quals:
            c._parent = comp
            self.dispatch(c,handler)
            if not first:
                self.graph.merge(comp,c)
                first=True
        #pdb.set_trace()
 
    def visitListCompFor(self,lcomp,handler):
        lcomp.list._parent=lcomp
        self.dispatch(lcomp.list,handler)
        self.graph.merge(lcomp, lcomp.list)

    def visitGenExpr(self,comp,handler):
        comp.code._parent=comp
        self.dispatch(comp.code,handler)
        self.graph.merge(comp,comp.code)
    

    def visitGenExprInner(self,comp,handler):
        first=False
        for c in comp.quals:
            c._parent = comp
            self.dispatch(c,handler)
    ...
    def visitGenExprFor(self,lcomp,handler):
        #pdb.set_trace()
        lcomp.iter._parent=lcomp
        self.dispatch(lcomp.iter,handler)
        self.graph.merge(lcomp, lcomp.iter)
    ...
    def visitFor(self, for_, handler):
        ...
        self.queueSourcePoint.append([for_,self.lastvisited()])
        ...
        for obj in lobj(getattr.expr, strict=1):
            ...
            if not attributes:
                ...
                try:
                    ...
                    if hasattr(attributes,"_namespace"):
                        attributes = obj._namespace.get(getattr.attrname, [])
                    #else:
                    #    pdb.set_trace()
    ...
    def visitIf(self, if_, handler):
        ...
        for compare, block in blocks:
            ...
            pre_exit.append(self.lastvisited())
        ...
        self.queueSourcePoint.append(pre_exit)
        ...
        if not hasattr(import_, "_names"):
            ...
            for module_name, as_name in import_.names:
                try:
                    ...
                    print "Could not import %s!"%module_name
                    pass
    ...
    def visitFrom(self, import_, handler):
        ...
        if not hasattr(import_, "_module"):
            import_._module = None
            try:
                module = self.session.process_name(import_.modname)
                import_._module=module                    
            except ImportError:
                print "Could not import %s!"%import_.modname
                pass

        _m=import_._module
        if _m:
            if len(import_.names)==1 and import_.names[0][0]=="*":
                import_.names=[(k,None) for k in _m._namespace.iterkeys()]
            for (_id,_as) in import_.names:
                if _id in _m._namespace:
                    for newast in _m._namespace[_id]:
                        newobj=self.graph.helper(newast, name=_id or _as, _parent=import_)
                        handler.store(newobj)

    def visitDict(self,dict,handler):
        "Process the given 'dict' node using the given 'handler'."
        self.depth_first(dict, handler)
        self.reset_specialisations(dict)

        dict_class = handler.builtins["dict"][0]

        # Make a reference to the class.
        # NOTE: This should be made dependent on the self argument.

        ref = self.instantiator.instantiate_class(dict_class, dict)
        args = [ref]

        # NOTE: Find the first __init__ method.

        if dict_class._namespace.has_key("__init__"):
            method = dict_class._namespace["__init__"][0]

            # Make a specialisation.

            self.call_specialisation(dict, method, args,
                "new", handler)

        # NOTE: Call the first append method.

        append_method = dict_class._namespace["__setitem__"][0]

        calls = getattr(dict, "_calls", [])
        for node, call in map(None, dict.items, calls):
            # Make a wrapper around the node as a place to add the call to the
            # append method.

            if call is None:
                call = self.graph.helper(node[0], _parent=dict, filename=self.filename)
                calls.append(call)

            self.reset_specialisations(call)
            method_args = [ref, node[0],node[1]]

            # Using top as refcontext to use the recently instantiated dict
            # object.
    ...
    def visitName(self, name, handler):
        ...
        #if 'name' in dir(name) and name.name=="set":
        #    self.visitSet(name,handler)
        #    return
        handler.load(name)
    ...
    def visitPrint(self, print_, handler):
        ...
        for node, call in map(None, print_.nodes, calls):
            ...
            for expr_input in lobj(node, strict=1):
                if hasattr(expr_input,"_namespace"):
    ...
    def visitReturn(self, return_, handler):
        ...
        #self.queueSourcePoint.append(return_)
        #pdb.set_trace()
    ...
    def visitStmt(self, stmt, handler):
        ...
        for child_node in child_nodes:
            ...
            # For convenience, add the parent node.
            if hasattr(handler,"skiprestofblock") and handler.skiprestofblock:
                handler.skiprestofblock=False
                break
    ...
    def visitSlice(self, slice, handler):
        ...
        for expr_input in lobj(slice.expr, strict=1):
            if hasattr(expr_input,"_namespace") and expr_input._namespace.has_key("__getslice__"):
                methods = expr_input._namespace["__getslice__"]
            elif hasattr(expr_input,"_class") and expr_input._class._namespace.has_key("__getslice__"):
    ...
    def visitSubscript(self, subscript, handler):
        ...
        for expr_input in lobj(subscript.expr, strict=1):
            if hasattr(expr_input,"_namespace") and expr_input._namespace.has_key("__getitem__"):
                methods = expr_input._namespace["__getitem__"]
            elif hasattr(expr_input,"_class") and expr_input._class._namespace.has_key("__getitem__"):
    ...
    def visitWhile(self, while_, handler):
        ...
        self.queueSourcePoint.append([while_,self.lastvisited()])
    ...
    def attach_true(self, node, arg, refcontext, handler):
        ...
        for expr_input in expr_inputs:
            ...
            elif hasattr(expr_input,"_class") and expr_input._class._namespace.has_key("__true__"):
    ...
    def attach_iteration(self, node, arg, refcontext, handler):
        ...
        for obj in objs:
            ...
            if hasattr(obj,"_namespace") and obj._namespace.has_key("__iter__"):
                methods = obj._namespace["__iter__"]
            elif hasattr(obj,"_class") and obj._class._namespace.has_key("__iter__"):
    ...
    def process_list_operator(self, operator, handler, strict=1):
        ...
        left_method_name, right_method_name = analysis.operators.get_list_methods(operator)
        
        L=operator.getChildren()
        
        for idx,op in enumerate(L[:-1]):
            l=op
            r=L[idx+1]
            self._process_binary_operator(operator, l, r, left_method_name, right_method_name, "left-right", "right-left", handler, strict)
    ...
    def _process_binary_operator(self, node, left, right, left_method_name, right_method_name, left_refcontext, right_refcontext, handler, strict=1):
        ...
        for left_input in left_inputs:
            ...
            methods=[]
            if hasattr(left_input,"_namespace") and left_input._namespace.has_key(left_method_name):
                methods = left_input._namespace[left_method_name]
            elif hasattr(left_input,"_class") and left_input._class._namespace.has_key(left_method_name):
        ...
        for (left_input, right_input) in incapable_combinations:
            ...
            if hasattr(right_input,"_namespace") and right_input._namespace.has_key(right_method_name):
                methods = right_input._namespace[right_method_name]
            elif hasattr(right_input,"_class") and right_input._class._namespace.has_key(right_method_name):
    ...
    def call_specialisation(self, caller, method, args, refcontext, handler, star_args=None, dstar_args=None):
        ...
        spec = self.specialiser.get_specialisation(caller, method, locals,  signature, True, False)
        if spec is None:
        ...
        if not (self.specialiser.is_signature_in_list(signature, caller._signatures) and spec in caller._targets):
            #caller.name=spec._qualified_name
            #pdb.set_trace()
    ...
    def _show_counter(self, spec):
        ...
        print >>sys.stderr
def contains(a,S):
    return hasattr(a,"_parent") and ((a._parent in S) or contains(a._parent,S))

def eval_node(self,node,handler=None):   
    #pdb.set_trace()
    #global callback
    self.prev=self.cp
    self.prevh=self.ch
    self.cp=node
    self.ch=handler
    if self.prev and not hasattr(self.prev,"_followedby"):
        self.prev._followedby=set()
    if self.cp and not hasattr(self.cp,"_followedby"):
        self.cp._followedby=set()
    #try:
    returning=False
    if self.prevh and self.prevh.return_nodes:
        if (not self.ch) or (not self.ch.return_nodes) or \
        set(self.prevh.return_nodes)-set(self.ch.return_nodes):
            #pdb.set_trace()
            self.queueReturn.append(self.prevh.return_nodes)
            returning=True

    def visit(f,t):
        if handler and handler.return_nodes and contains(f,set(handler.return_nodes)):  
            if returning:
                pass
            else:
                return
        if not hasattr(f,"_uid"):
            self.session.maxuid+=1
            f._uid=self.session.maxuid
        if not hasattr(t,"_uid"):
            self.session.maxuid+=1
            t._uid=self.session.maxuid
        if self.session.callback:
            self.session.callback(f,t)
        f._followedby.add(t)

    if self.session.pre:
        if self.queueReturn:
            lst=self.queueReturn.pop()
            for l in lst:
                visit(l,self.cp)
        if self.queueSourcePoint:
            lst=self.queueSourcePoint.pop()
            for l in lst:
                visit(l,self.cp)
        if self.prev:
            visit(self.prev,self.cp)
        returning = False
    ...
    #    print e
    #    pass
    funName="new_visit"+node.__class__.__name__
    if not hasattr(self,funName):
        funName="new_default"
    code="self.%s(node,handler)"%funName
    res=eval(code)    
    if not self.session.pre:
        if self.queueReturn:
            lst=self.queueReturn.pop()
            for l in lst:
                visit(l,self.cp)
...
class AbstractVisitor(AnalysisVisitor):
    def __init__(self, session=None, filename=None, module_name=None):
        AnalysisVisitor.__init__(self,session,filename,module_name)
        self.prev=None
        self.cp=None
        self.ch=None
        self.prevh=None
        ...
        for name in dir(self):
            if (len(name)>5 and name[:5]=="visit") or name=="default":
                if "new_"+name not in dir(self):
                    exec "self.__class__.%s=new.instancemethod(self.__class__.%s,None,self.__class__)"%("new_"+name,name) in globals(),locals()
                    exec "self.__class__.%s=new.instancemethod(eval_node,None,self.__class__)"%name
...
class AnalysisSession:
    ...
    def __init__(self,**kwargs):
        ...
        self.pre=True #Pretraversal CFG construction
        self.maxuid=0
        self.librarypath="."
        self.nestedlevel=0
        self.lastfunc=None
        self.codepath="."
        self.simplify_files=False
    ...
    def process_builtins(self):
        ...
        if self.builtins is None:
            filename = os.path.join(self.librarypath,"lib", "builtins.py")
    ...
    def processAST(self, ast,filename=""):
    ...
    def visit_cfg_setup(self,ast,kwargs):
        self.maxuid=0
        self.pre="post_traversal" in kwargs and not kwargs["post_traversal"]
        if "cfg_visitor" in kwargs:
            cfg_visitor=kwargs["cfg_visitor"]           
            if cfg_visitor:
                if "only_source_ast" in kwargs:
                    only_source_ast=kwargs["only_source_ast"]
                    if only_source_ast:
                        def auxvisit(f,t):
                            leaving=not (issubast(t,ast) or (hasattr(t,"filename") and "builtins" not in t.filename))
                            left=not (issubast(f,ast) or (hasattr(f,"filename") and "builtins" not in f.filename))
                            #leaving=not (issubast(t,ast))
                            #left=not (issubast(f,ast))
                            if leaving and left: #we are out side the given AST
                                #pdb.set_trace()
                                pass
                                #do nothing
                            elif leaving and not left: #we are leaving the given AST
                                self.lastvisited=f
                            elif not leaving and not left: #we are inside the given AST
                                cfg_visitor(f,t)
                                self.lastvisited=t
                            else: #we are coming back to the AST       
                                cfg_visitor(self.lastvisited,t)
                                self.lastvisited=t                       
                        self.callback=auxvisit
                    else:
                        self.callback=cfg_visitor
                else:
                    self.callback=cfg_visitor

    def processAST_to_CFG(self, ast,filename="",**kwargs):
        "Process the string 's', returning a module root node."
        self.process_builtins()
        module = ast
        self.processed_files.append(filename)        
        compiler.misc.set_filename(filename,module)
        visitor = AbstractVisitor(self)
        self.visit_cfg_setup(module,kwargs)        
        compiler.walk(module, visitor, visitor)
        return module

    def processFile_to_CFG(self, filename,**kwargs):
        ...
        if not self.simplify_files:
            module = compiler.parseFile(filename)
        else:
            #print "Simplifying %s"%filename
            from plugins.simplify import simplify
            res=simplify(filename)
            module=compiler.parse(res)
        self.processed_files.append(filename)
        module_name = self.get_module_name(filename)
        self.visit_cfg_setup(module,kwargs)
        visitor = AbstractVisitor(self, filename=filename, module_name=module_name)
        compiler.walk(module, visitor, visitor)
    ...
    def process_name(self, module_name):
        ...
        if hasattr(self,"path"):
            dirs.append(os.path.split(self.path)[0])            
        dirs.append(os.path.join(self.librarypath,"lib"))
        dirs.append(self.codepath)
        oldmodname=module_name
        module_name=os.path.join(*module_name.split("."))
        print "  "*(self.nestedlevel)+"Importing %s"%oldmodname
        for d in dirs:
            ...
            if os.path.exists(pathname):
                ...
                if hasattr(self,"path"):
                    oldpath=self.path
                self.path=pathname#os.path.abspath(pathname)
                self.nestedlevel+=1
                print ("  "*(self.nestedlevel-1)+" processing %s ..."%pathname)
                res=self.process_file(pathname)
                print "  "*(self.nestedlevel-1)+" done."
                self.nestedlevel-=1
                print "  "*(self.nestedlevel)+"Success!"
                if oldpath:
                    self.path=oldpath
                else:
                    del self.path
                return res
        print "  "*(self.nestedlevel)+"ERROR - module %s could not be found!"%oldmodname
    ...
    def simplify(self,filename):
        ...
        res=simplify(filename)
        self.processed_files.append(filename)
        self.path=os.path.join(os.path.split(filename)[0],'.')
        compobj = compiler.compile(res,filename,'exec')
        #exec compobj
        return res
    def save(self,object,filename):
        sys.setrecursionlimit(1024000)
        ds = []
        for i in self.processed_files:
            if i!="":
                o=open(i,'r')
                ds.append((i,o.read()))
                o.close()
        o=open(filename,'w')
        cPickle.dump((ds,object),o)
        o.close()
    def load(self,filename):
        sys.setrecursionlimit(1024000)
        o=open(filename,'r')
        (ds,object) = cPickle.load(o)
        o.close()
        for (fname,cont) in ds:
            o=open(fname,'r')
            if o.read()!=cont:
                o.close()
                raise Exception,"File %s does not match the stored file!"%fname
            o.close()
        return object
...
def issubast(needle,haystack):
    if needle==haystack:
        return True
    if not hasattr(needle,'_parent'):
        return False
    return issubast(needle._parent,haystack)
