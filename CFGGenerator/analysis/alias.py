...
class Graph:
  ...
  def revVisit(self,action,start=None,stopcondition=lambda f: False):
    visited=set()
    W=set()
    if start==None:
        start=self.start
        ...
        if hasattr(N,"_postexec"):
            ...
            for E in N._P:
                if hasattr(E,"_postexec"):
                    ...
                    return (self.makeNode(E,NC),0)
        pdb.set_trace()
    def contains(self,NodeID,X,Y,cr=[0,0,0,0]):
        x=self.getStorage(X)
        y=self.getStorage(Y)
        if x in self.FDSet[NodeID] and y in self.FDSet[NodeID][x]:
            return False
        ...
        while self.hasOnePred(NodeID):
            ...
            if self.hasOneSucc(pred): #Look up in prevs
                ...
                if x in self.FDSet[NodeID] and y in self.FDSet[NodeID][x]:
                    if cr[0]%1000==0:
                        print "F", cr[0],cr[1],cr[2],cr[3]
                    cr[0]+=1
                    cr[1]=(cr[1]*cr[0]+c+1.0)/cr[0]
                    return False
                if x in self.FSet[NodeID] and y in self.FSet[NodeID][x]:
                    if cr[0]%1000==0:
                        print "F", cr[0],cr[1],cr[2],cr[3]
        ...
        #return x in self.FSet[NodeID] and y in self.FSet[NodeID][x]
        return False
    def rcontains(self,NodeID,X,Y,cr=[0,0,0,0]):
        x=self.getStorage(X)
        y=self.getStorage(Y)
    ...
    def worksetalgo(self):
        ...
        def tracecompress(tracenodes):
            ...
            for node in tracenodes.keys():
                ...
                self.saw=0
                self.done=False
                self.G.revVisit(F,node,lambda x : self.done)
