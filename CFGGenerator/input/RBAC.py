...
# supporting system functions
    ...
    def CheckAccess(self, session, operation, object):
        ...
        return (set(r for r in self.ROLES if (session,r) in self.SR and ((operation,object),r) in self.PR))
...
# advanced review functions
    ...
    # SSD constraint
    def checkSSD(self):
        return not (set(u for u in self.USERS for (name,c) in self.SsdNC
...
#alt    return set(r for r in self.ROLES if (name,r) in self.SsdNR)
   ...
   # DSD constraint, as in CoreRBACwithSSD and GeneralHierRBACwithSSD,
    ...
    def checkDSD(self):
        return not (set(s for s in self.SESSIONS for (name,c) in self.DsdNC
                            if len(set(r for r in set(r for r in self.ROLES if (s,r) in self.UR)
